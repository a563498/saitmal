...
export async function onRequestGet({ request, env }){
  try{
    ...
    const answer = a.data;
    const guess = g.data;
    const nonExact = !!guess.__nonExact;
    ...
    return jsonResponse({ ok:true, data:{ word: guess.word, similarity, isCorrect, nonExact, clues }});
  }catch(e){
    return jsonResponse({ ok:false, message:"서버 오류(guess)", detail:String(e && e.stack ? e.stack : e) }, 500);
  }
}
