...
export async function lookupWord(env, word){
  ...
  u.searchParams.set("num", "10"); // IMPORTANT: must be numeric, else error 103
  u.searchParams.set("sort", "dict");
  u.searchParams.set("advanced", "y");
  u.searchParams.set("type1", "word");
  u.searchParams.set("pos", "1");
  ...
  if (!item || !item.word){
    // fallback: view by searching first item (non-exact)
    const u2 = new URL(OPENDICT_BASE);
    u2.searchParams.set("key", env.OPENDICT_KEY);
    u2.searchParams.set("q", q);
    u2.searchParams.set("req_type", "xml");
    u2.searchParams.set("num", "1");
    const s2 = await fetchXml(u2.toString());
    if (!s2.ok || /<error>/i.test(s2.text) ){
      return { ok:false, status:404, message:"검색 결과가 없어요." };
    }
    item = parseSearch(s2.text, null);
  }
...
export async function pickDailyAnswer(env){
  ...
  const seeds = ["사과","사람","바다","도시","기차","학교","친구","여행","커피","음악","강아지","고양이","산책","영화","책","바람","겨울","여름","하늘","별"];
