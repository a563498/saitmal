...
async function fetchJsonSafe(url){
  const r=await fetchTextSafe(url);
...
async function opendictSearch(env, q, opts={}){
  ...
  const p=new URLSearchParams({
    key:env.OPENDICT_KEY,q,req_type:"json",advanced:"n",part:"word",
    method:opts.method||"exact",start:String(opts.start||1),num:String(opts.num||30),sort:opts.sort||"dict"
  });
  const r=await fetchJsonSafe(`${API_SEARCH}?${p.toString()}`);
