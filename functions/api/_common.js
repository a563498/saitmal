...
function parseSearchXml(xml, wordExact=null){
  const words = xmlGetAll(xml, "word").map(stripCaret);
  if (!words.length) return null;
  let idx=0;
  if (wordExact){
    const norm = stripCaret(wordExact);
    const found = words.findIndex(w=>w===norm);
    if (found>=0) idx=found;
  }
  const pos = xmlGetAll(xml,"pos")[idx] || "";
  const definition = xmlGetAll(xml,"definition")[idx] || "";
  const target_code = xmlGetAll(xml,"target_code")[idx] || null;
  const type = xmlGetAll(xml,"type")[idx] || "";
  const cat = xmlGetAll(xml,"cat")[idx] || "";
  return { word: words[idx], pos, definition, target_code: target_code ? Number(target_code) : null, type, cat };
}
function parseViewXml(xml){
  const word = (xmlGetAll(xml,"word")[0]||"").trim();
  const pos = (xmlGetAll(xml,"pos")[0]||"").trim();
  const definition = (xmlGetAll(xml,"definition")[0]||"").trim();
  return { word: stripCaret(word), pos, definition };
}
...
async function lookupWord(env, word){
  ...
  const item=pickEntryFromSearch(s.data,w);
  if (!item) return {ok:false,status:404,message:`"${w}" 검색 결과 없음`};

  const out={word:stripCaret(item.word),pos:item.pos||"",definition:item.definition||"",target_code:item.target_code??null,type:item.type||"",cat:item.cat||item.cat_info?.cat||""};
  ...
  if (!out.definition) return {ok:false,status:502,message:"뜻풀이를 가져오지 못했어요(사전 응답 형식 문제)."};
...
async function pickDailyAnswer(env, gameId=null){
  ...
  const ans={word:stripCaret(picked.word),pos:picked.pos||"",definition:picked.definition||"",target_code:picked.target_code??null,type:picked.type||"",cat:picked.cat||picked.cat_info?.cat||""};
