...
export function getOpenDictKey(env){
...
function parseSearch(xml, exact){
  ...
  function normItem(block){
    ...
    // definition can appear nested under <sense> in search response
    let def = "";
    const defm = block.match(/<definition>([\s\S]*?)<\/definition>/i);
    if (defm) def = decodeEntities(defm[1].replace(/<!\[CDATA\[([\s\S]*?)\]\]>/g,"$1")).trim();

    // target_code is required for view fallback
    let tc = null;
    const tcm = block.match(/<target_code>([\s\S]*?)<\/target_code>/i) || block.match(/<targetCode>([\s\S]*?)<\/targetCode>/i);
    if (tcm){
      tc = decodeEntities(tcm[1].replace(/<!\[CDATA\[([\s\S]*?)\]\]>/g,"$1")).trim() || null;
    }
...
export async function lookupWord(env, word){
  ...
  if (!item.definition){
    return { ok:false, status:502, message:"뜻풀이를 가져오지 못했어요(사전 응답 형식 문제).", detail:{ reason:"definition empty", searchSnippet: sres.text.slice(0,260), parsed:{ word:item.word, pos:item.pos, target_code:item.target_code } } };
  }
