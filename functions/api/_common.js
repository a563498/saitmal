...
async function lookupWord(env, word){
  ...
  if (!out.definition){
    // 일부 단어는 검색 응답이 매우 축약되거나, 일시적으로 definition이 비어올 수 있습니다.
    // 게임이 멈추지 않도록 “뜻풀이 없음”으로 처리하고, 점수는 낮게 나오게 됩니다.
    return {ok:true,data:{...out, definition:"", noDefinition:true}};
  }
...
async function pickDailyAnswer(env, gameId=null){
  ...
  const seedBase = gameId ? `game:${gameId}` : `date:${dateKey}`;
  let h = fnv1a32(seedBase);

  const syllables = ["가","나","다","라","마","바","사","아","자","차","카","타","파","하"];

  // 최대 6회 다른 쿼리로 재시도(정답 후보/뜻풀이 누락/업스트림 오류 완화)
  for (let attempt=0; attempt<6; attempt++){
    const q = syllables[h % syllables.length];
    const startNo = 1 + ((h >>> 8) % 800);

    const s = await opendictSearch(env, q, { method:"include", start: startNo, num: 30, sort:"dict" });
    if (!s.ok){
    ...
    for (let k=0; k<arr.length; k++){
      ...
      const ans = {
        ...
        target_code: picked.target_code ?? picked.targetCode ?? null,
        type: picked.type || "",
  ...
  return { ok:false, status: 502, message: "정답 뜻풀이를 가져오지 못했어요.", detail: "OpenDict 응답에서 definition/target_code가 비어있는 항목만 반환되거나, 업스트림 오류가 반복되었습니다. OPENDICT_KEY 유효/호출 제한 여부 확인이 필요합니다." };
}
