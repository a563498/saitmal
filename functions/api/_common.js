...
function tagValue(xml, tag){
  ...
  if (!m) return "";
  let v = m[1];
  // strip CDATA without regex pitfalls
...
function parseSearch(xml, exact){
  ...
  function normItem(block){
    ...
    const tcRaw = decodeEntities(tagValue(block,"target_code")) || decodeEntities(tagValue(block,"targetCode"));
    const tc = tcRaw ? tcRaw.trim() : null;
  ...
  if (want){
    ...
    return { __noExact:true, first:normItem(items[0]) }; // exact not found
  }
...
function parseView(xml){
  ...
  const definition = decodeEntities(tagValue(item,"definition"));
...
export async function lookupWord(env, word){
  ...
  if (item && item.__noExact){
    item = item.first;
    item.__nonExact = true;
  }
  if (!item || !item.word){
    // return a snippet for debugging (first 400 chars)
    return { ok:false, status:404, message:"검색 결과가 없어요.", detail:{ q, snippet: sres.text.slice(0,400) } };
  }
