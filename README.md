...
### v14 (게임성/신뢰도 보정)
- **TOP 버튼/모달 표기**를 `TOP 10`으로 통일 (UI 텍스트 불일치 수정)
- 우측 상단 **DB 기준 최고 유사(정답 제외)**가 0%로 고정되던 문제 수정
  - 초기 로딩 시 `/api/top?limit=1`을 호출해 Top1 %를 가져와 `최고 유사`에 표시
- 입력 누적 정렬 규칙 확정
  - **가장 최근 입력 단어 1개는 항상 맨 위로 고정**
  - 나머지는 **유사도 높은 순**(동률이면 더 최근이 위)
- `/api/guess` 응답 단순화
  - 글자수/품사/난이도 힌트(클루) 제거 → 유사도 추론에 집중
- 유사도 로직 보정(“사과가 3%” 같은 납득 어려운 결과 완화)
  - 시간/즉시/짧은 간격 계열 컨셉 매핑 확장 (`시간/때/시각/잠깐/조금/... → 짧은시간`)
  - 우연한 토큰 겹침으로 생기는 낮은 점수는 **0%로 눌러** 노이즈 감소
...
### v15~v16 (TOP 계산 안정화)
- D1의 “too many SQL variables” 오류를 피하도록 **TOP 계산을 랜덤 샘플 기반**으로 제한하고 KV 캐시를 강화
- `/api/top` 응답 형식 단순화(목록만 반환)

### v17 (요청 반영: Top 호출 없이 즉시 반영)
- `/api/meta` 응답에 `bestDB`(정답 제외, DB 기준 Top1 %)를 포함
- `TOP` 결과는 서버에서 **KV 캐시 우선**으로 제공
- 프론트는 초기 로딩 시 별도의 `/api/top?limit=1` 호출 없이도 우측 상단 “최고 유사”가 바로 표시됨
